...
const inputErrStyle: React.CSSProperties = {
  ...inputStyle, border: '1px solid rgba(239,68,68,0.4)', background: 'rgba(239,68,68,0.06)',
}
...
export default function ClientEdit() {
  ...
  useEffect(() => {
    ...
    }).catch(() => setLoadError('Não foi possível carregar os dados do cliente.'))
  }, [id, reset])

  async function onSubmit(values: FormValues) {
    if (!id) return; setSaving(true); setSaveError('')
    try {
      await clientsApi.update(id, {
        ...
        origin: values.origin, notes: values.notes || null,
      })
      navigate(`/clients/${id}`)
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { error?: string } } })?.response?.data?.error
      setSaveError(msg ?? 'Erro ao salvar alterações')
  ...
  const tabContent: React.CSSProperties = {
    background: '#0B1F30', border: '1px solid rgba(0,200,232,0.14)',
    borderRadius: '0 12px 12px 12px', padding: 24,
  }

  if (loadError) return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="rounded-lg px-4 py-3 text-sm" style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', color: '#f87171' }}>{loadError}</div>
    </div>
  )
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <Link to={`/clients/${id}`} className="text-sm hover:underline" style={{ color: '#7EAFC4' }}>← Voltar</Link>
        <span style={{ color: '#3E6A80' }}>/</span>
        <h1 className="text-xl font-bold" style={{ color: '#E8F4F8' }}>Editar cliente</h1>
      </div>
        ...
        <Tabs.Root value={activeTab} onValueChange={setActiveTab}>
          ...
          <Tabs.Content value="business" style={tabContent}>
            <div className="space-y-4">
              ...
              <div><Label>Nicho</Label><Select {...register('niche')}><option value="">Selecionar nicho...</option>{NICHOS.map(n => <option key={n.value} value={n.value}>{n.label}</option>)}</Select></div>
              <div><Label>Bairro</Label><Select {...register('neighborhood')}><option value="">Selecionar bairro...</option>{BAIRROS_ZONA_LESTE.map(b => <option key={b} value={b}>{b}</option>)}</Select></div>
              <div><Label>Endereço completo</Label><Input {...register('address')} placeholder="Rua, número, complemento" /></div>
              <div className="flex justify-end pt-2">
                <button type="button" onClick={() => setActiveTab('contact')} className="px-5 py-2 rounded-lg text-sm font-bold" style={{ background: 'linear-gradient(135deg, #00C8E8, #00E5C8)', color: '#061422' }}>Próximo →</button>
              </div>
            </div>
          </Tabs.Content>
          ...
          <Tabs.Content value="contact" style={tabContent}>
            <div className="space-y-4">
              ...
              <div className="flex justify-between pt-2">
                ...
                <button type="button" onClick={() => setActiveTab('contract')} className="px-5 py-2 rounded-lg text-sm font-bold" style={{ background: 'linear-gradient(135deg, #00C8E8, #00E5C8)', color: '#061422' }}>Próximo →</button>
              </div>
            </div>
          </Tabs.Content>
          ...
          <Tabs.Content value="contract" style={tabContent}>
            <div className="space-y-4">
              ...
              <div><Label>Data de início</Label><Input {...register('startDate')} type="date" /></div>
              ...
              {watchOrigin === 'referral' && <div><Label>Quem indicou?</Label><Input {...register('notes')} placeholder="Nome do indicador" /></div>}
              <div>
                <Label>Observações</Label>
                <textarea {...register('notes')} rows={3} placeholder="Anotações internas, acordos especiais..."
                  style={{ ...inputStyle, resize: 'none' }} />
              </div>
              {saveError && <div className="rounded-lg px-4 py-3 text-sm" style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', color: '#f87171' }}>{saveError}</div>}
              <div className="flex justify-between pt-2">
                <button type="button" onClick={() => setActiveTab('contact')} className="text-sm px-4 py-2" style={{ color: '#7EAFC4' }}>← Voltar</button>
                <button type="submit" disabled={saving} className="px-6 py-2 rounded-lg text-sm font-bold flex items-center gap-2 disabled:opacity-60"
                  style={{ background: 'linear-gradient(135deg, #00C8E8, #00E5C8)', color: '#061422' }}>
                  {saving ? <><span className="w-4 h-4 rounded-full animate-spin" style={{ border: '2px solid rgba(6,20,34,0.3)', borderTop: '2px solid #061422' }} />Salvando...</> : '✓ Salvar alterações'}
                </button>
              </div>
            </div>
          </Tabs.Content>
