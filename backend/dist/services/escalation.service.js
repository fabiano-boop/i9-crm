...
// ─── Core trigger ─────────────────────────────────────────────────────────────
async function triggerEscalation(payload) {
    const urgency = payload.urgencyLevel ?? 'medium';
    logger_js_1.logger.info({ leadId: payload.leadId, reason: payload.reason, urgency }, 'Escalação: lead encaminhado para humano');
    ...
    if (env_js_1.env.RESEND_API_KEY) {
        await sendEscalationEmail(payload, urgency).catch((err) => logger_js_1.logger.warn({ err }, 'Escalação: falha ao enviar email — continuando'));
    }
    else {
        logger_js_1.logger.warn('Escalação: RESEND_API_KEY não configurado — email de alerta não enviado');
    }
}
// ─── Email alert ──────────────────────────────────────────────────────────────
async function sendEscalationEmail(payload, urgency) {
  ...
  <body style="margin:0;padding:0;background:#f9fafb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
    <div style="max-width:600px;margin:32px auto;background:white;border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.1);">
      ...
      <div style="padding:24px;text-align:center;">
        <a href="${env_js_1.env.FRONTEND_URL}/agent"
    ...
    logger_js_1.logger.info({ leadId: payload.leadId, recipients: recipients.length }, 'Escalação: email enviado com sucesso');
}
