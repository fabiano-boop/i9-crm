...
async function sendWhatsAppReply(phone, message) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            await axios_1.default.post(`${env_js_1.env.WHAPI_URL}/messages/text`, { to, body: message }, { headers: { Authorization: `Bearer ${env_js_1.env.WHAPI_TOKEN}` }, timeout: 15_000 });
            return true;
        }
        catch (err) {
            logger_js_1.logger.warn({ phone: to, attempt, err: err instanceof Error ? err.message : err }, 'Agente: falha ao enviar');
            if (attempt < 3)
