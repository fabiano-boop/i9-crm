...
async function sendReport(reportId) {
    ...
    if (client.whatsapp && env_js_1.env.EVOLUTION_API_URL && env_js_1.env.EVOLUTION_API_KEY) {
        try {
            const pdfUrl = report.pdfPath
                ? `${env_js_1.env.REPORTS_BASE_URL}/api/clients/${client.id}/reports/${reportId}/pdf`
                : null;
            const message = buildWhatsAppMessage(client, report, readRate, pdfUrl);
            const axios = (await import('axios')).default;
            await axios.post(`${env_js_1.env.EVOLUTION_API_URL}/message/sendText/${env_js_1.env.EVOLUTION_INSTANCE_NAME}`, { number: client.whatsapp, text: message }, { headers: { apikey: env_js_1.env.EVOLUTION_API_KEY, 'Content-Type': 'application/json' } });
            await database_js_1.prisma.weeklyReport.update({ where: { id: reportId }, data: { sentViaWhatsApp: true, sentAt: new Date() } });
            logger_js_1.logger.info({ reportId, whatsapp: client.whatsapp }, 'Relatório enviado por WhatsApp');
        }
        catch (err) {
            logger_js_1.logger.warn({ err, reportId }, 'Falha ao enviar relatório por WhatsApp');
        }
    }
}
// ─── Helpers de mensagem ──────────────────────────────────────────────────────
function buildWhatsAppMessage(client, report, readRate, pdfUrl) {
    const lines = [
