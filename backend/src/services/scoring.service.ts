...
export async function scoreLead(leadId: string): Promise<ScoringResult> {
  ...
  try {
    ...
    result = JSON.parse(jsonMatch[0]) as ScoringResult
  } catch (err) {
    logger.error({ leadId, rawText, err }, 'Falha ao parsear resposta do Claude')
  ...
  logger.info({ leadId, businessName: lead.businessName, score: result.score, classification: result.classification, reasoning: result.reasoning }, 'Lead rescorado')

  return result
}

export async function bulkScoreLeads(leadIds: string[]): Promise<{ id: string; result?: ScoringResult; error?: string }[]> {
  ...
  for (const id of ids) {
    try {
      ...
      logger.warn({ id, error }, 'Erro ao rescorar lead')
    }
  }
